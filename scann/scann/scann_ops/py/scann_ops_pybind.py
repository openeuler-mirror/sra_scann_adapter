...
class ScannSearcher(object):
  ...
  def serialize_to_mem(self):
    serialized_data = self.searcher.serialize_to_numpy()
    return serialized_data

  def get_num(self):
    num = self.searcher.getnum()
    return num
  
  def get_dim(self):
    dim = self.searcher.getdim()
    return dim
...
def builder(db, num_neighbors, distance_measure):
  ...
  return scann_builder.ScannBuilder(db, num_neighbors,
...
def deserialize_from_mem(serialized_data):
  return ScannSearcher(scann_pybind.ScannNumpy(serialized_data))