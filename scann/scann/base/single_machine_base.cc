...
SingleMachineSearcherBase<T>::~SingleMachineSearcherBase() {
  if (!so_path_.empty()) {
    if (std::remove(so_path_.c_str()) != 0) {
        std::cerr << "Warning: Failed to delete file " << so_path_ << std::endl;
    }
  }
}
...
Status SingleMachineSearcherBase<T>::FindNeighborsBatched(
  ...
  if (reordering_helper_) {
    for (DatapointIndex i = 0; i < queries.size(); ++i) {
      SCANN_RETURN_IF_ERROR(ReorderResults(queries[i], params[i], &results[i], false));
    }
  }
  ...
  if (reordering_helper_) {
    for (DatapointIndex i = 0; i < queries.size(); ++i) {
      SCANN_RETURN_IF_ERROR(ReorderResults(queries[i], params[i], &results[i], true));
    }
  }
...
Status SingleMachineSearcherBase<T>::ReorderResults(
    const DatapointPtr<T>& query, const SearchParameters& params,
    NNResultsVector* result, bool accurate) const {
...
#ifdef FOR_MILVUS
#else
  } else if (kReorderHelper->GetMode() == KReorderHelperClass::PMODE::READY && !accurate) {
