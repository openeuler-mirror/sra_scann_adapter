...
vector<std::vector<QueryForLeaf>> InvertCentersToSearch(
  ...
  for (DatapointIndex query_index : IndicesOf(centers_to_search)) {
    ...
    for (int i=0; i<pred_result[query_index]; i++) {
      result[cur_query_centers[i].first].emplace_back(query_index,
                                                cur_query_centers[i].second);
    }
  }
  return result;
} 
...
Status TreeAHHybridResidual::FindNeighborsBatchedImpl(
  ...
  if (pAdaptiveModel->GetMode() == IadpModel::AMODE::INFERENCE) {
    ...
    queries_by_leaf =
      InvertCentersToSearch(centers_to_search, maybe_projected_query_tokenizer_->n_tokens(), pred_result);
  } else {
#endif
    queries_by_leaf =
      InvertCentersToSearch(centers_to_search, maybe_projected_query_tokenizer_->n_tokens());
#ifdef FOR_MILVUS
#else
  }
#endif
...
bool TreeAHHybridResidual::SearchCenters(const DatapointPtr<float>& query, 
      const int num_centers, vector<pair<DatapointIndex, float>> &centers_to_search) {
